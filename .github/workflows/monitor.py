...
def format_info(tracked_info, priority_1_count, total_count):
    lines = [
        f"📊 <b>СВОДКА ПО ИТМО</b>",
        f"═" * 25,
        f"👥 <b>Всего:</b> {total_count}",
        f"⭐ <b>Приоритет 1:</b> {priority_1_count}",
        ""
    ]
    for num, info in tracked_info.items():
        ...
        lines.append(f"   📊 Балл: <b>{info['total_score']}</b>")
...
def main():
    ...
    html = get_page()
    if not html:
        send_telegram("⚠️ Ошибка загрузки")
        return
    
    new_data = parse_data(html)
    if not new_data:
        send_telegram("⚠️ Ошибка парсинга")
        return
    ...
    if old_data:
        ...
        if len(old_data) != len(new_data):
            send_telegram(f"🔔 <b>ИЗМЕНИЛОСЬ КОЛИЧЕСТВО!</b>\n{len(old_data)} → {len(new_data)}")
        elif changes:
            send_telegram("🔔 <b>ИЗМЕНЕНИЯ!</b>\n\n" + "\n".join(changes))
    else:
        print("✅ Первый запуск")
